        ...
        <ul className="m-0 flex-1 list-none space-y-2 p-0" aria-labelledby="top-news-heading">
          {TOP_NEWS.map((item) => (
            <li key={item.date + item.title}>
              <Link
                href={item.href}
                className="group flex min-h-[44px] flex-wrap items-center gap-2 rounded-md py-1 text-[15px] text-[var(--color-text)] no-underline hover:bg-[#f6f8fa] hover:text-[var(--kasumi-blue)] md:gap-3"
              >
